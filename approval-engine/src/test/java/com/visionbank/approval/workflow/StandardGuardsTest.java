...
class StandardGuardsTest {
    ...
    private PolicySnapshot policy(int required, boolean makerCanApprove) {
        return new PolicySnapshot("v1", Map.of("PENDING_APPROVAL", new StagePolicy(required, List.of("TRANSFER_CHECKER"))), makerCanApprove);
    }

    @Test
    void noApprovalRequiredWhenRequiredApprovalsIsZero() {
        GuardContext ctx = new GuardContext("maker-1", policy(0, false), 0, null, null, false, "PENDING_APPROVAL");
    ...
    void approvalRequiredWhenRequiredApprovalsPositive() {
        GuardContext ctx = new GuardContext("maker-1", policy(2, false), 0, null, null, false, "PENDING_APPROVAL");
    ...
    void approvalsSatisfiedComparesCountToRequired() {
        GuardContext under = new GuardContext("maker-1", policy(2, false), 1, null, null, false, "PENDING_APPROVAL");
        GuardContext at = new GuardContext("maker-1", policy(2, false), 2, null, null, false, "PENDING_APPROVAL");
    ...
    void actorIsMakerComparesActorIdToMakerId() {
        GuardContext ctx = new GuardContext("maker-1", policy(1, false), 0, "maker-1", "MAKER", false, "PENDING_APPROVAL");
        GuardContext other = new GuardContext("maker-1", policy(1, false), 0, "checker-1", "TRANSFER_CHECKER", false, "PENDING_APPROVAL");
    ...
    void actorIsEligibleCheckerComparesRoleToPolicyRoles() {
        GuardContext eligible = new GuardContext("maker-1", policy(1, false), 0, "checker-1", "TRANSFER_CHECKER", false, "PENDING_APPROVAL");
        GuardContext ineligible = new GuardContext("maker-1", policy(1, false), 0, "someone", "AUDITOR", false, "PENDING_APPROVAL");
    ...
    void slaExpiredReflectsContextFlag() {
        GuardContext expired = new GuardContext("maker-1", policy(1, false), 0, null, null, true, "PENDING_APPROVAL");
        GuardContext notExpired = new GuardContext("maker-1", policy(1, false), 0, null, null, false, "PENDING_APPROVAL");
